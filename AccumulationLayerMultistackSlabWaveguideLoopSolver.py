...
out_path = os.path.join(dir_paths["slabwg"], f'{wg[2]}_{l1[2]}_ALT_RI_Loop')
org.check_dir_exists(out_path)

# Parameters
alt = np.arange(0, l2[1]+0.5, 0.5)
delta_n = np.arange(l2[0]-1, l2[0]+0.01, 0.01)
thicknesses = [np.array([sub[2], wg[2], l1[2] - x, x, cov[2]]) for x in alt]

for dn in delta_n:
    indices = [sub[0], wg[0], l1[0], round(dn, 4), cov[0]]
    k0 = 2 * np.pi / lm0
    n_guesses = np.arange(cov[0], wg[0], 0.0001)
